...
function main() {
  ...
  do
    ...
    joborder=$( ciop-copy -O ${TMPDIR} ${jobroder_ref} ) 
	...
	for mydoris in `cat $input | sort -u | grep "^.or=" | cut -d "=" -f 2-`
	do
		enclosure=$( opensearch-client $mydoris enclosure )
		ciop-copy -O $TMPDIR/aux/ENVI/Doris ${enclosure}
		[ "$?" != "0" ] && exit $ERR_AUX
	done
	 
	ciop-log "INFO" "copying ASAR aux"
	for myaux in `cat $input | sort -u | grep "^aux=" | cut -d "=" -f 2-`
	do
		enclosure=$( opensearch-client $myaux enclosure )
